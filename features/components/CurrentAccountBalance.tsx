...
const CurrentAccountBalance = () => {
  ...
  const handleInputExpenseChange = ({
    target: { value },
  ...
  return (
    <Box bg="#E6FAFD" borderRadius="3xl" h={540} w={850}>
      ...
      <Text paddingLeft={390} fontSize={25}>
        {currentMoney}
      </Text>
      <HStack paddingTop={85} paddingLeft={130}>
        <Box>
          <VStack>
            ...
            <Button onClick={() => handleCurrentAmounOfMoney("income")}>
              Dodaj przychód
            </Button>
          </VStack>
        </Box>
        <Box>
          <VStack>
            <Input
              ...
              placeholder="wpisz kwote"
              width="25"
              onChange={handleInputExpenseChange}
            />
            <Select
              placeholder="wybierz rodzaj wydatku"
              onChange={({ target: { value } }) => setTypeOfSource(value)}
            >
              <option key="test-1" value="rozrywka">
                rozrywka
              </option>
              <option key="jedzenie" value="jedzenie">
                jedzenie
              </option>
              <option key="domowe" value="domowe">
                domowe
              </option>
            </Select>
            <Button onClick={() => handleCurrentAmounOfMoney("expense")}>
              Dodaj wydatek
            </Button>
          </VStack>
        </Box>
